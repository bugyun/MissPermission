...
import com.ruoyun.dpermission.check.CameraChecker;
 ...
 * Depiction:
 */
public class DPermissionHelper {
    ...
    public static void doOpenCamera(final Activity activity, final DoAction doAction) {
        DPermission.with(activity).addPermission(Manifest.permission.CAMERA).checkPermission(new PermissionRequest.PermissionListener() {
            @Override
